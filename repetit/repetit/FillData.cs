...
namespace repetit
{
	...
	public class DkorExampleTest
	{
		...
		[Test]
		public void AddLessonsForThisWeek()
		{
			Login();
			Thread.Sleep(500);
			...
			for (var i = 0; i < pupilsCount; i++)
			{
				...
				var currentMonth = DateTime.Today.Month;
				var pDay = Tools.ThisMonday().AddDays(pCurrent.DayOfWeek - 1);
			...
			Console.WriteLine("");
			Console.WriteLine($"Добавлены занятия для {pupilsCount} учеников на текущую неделю");
		...
		[Test]
		public void DeleteAllLessonsForThisWeek()
		{
			Login();
			Thread.Sleep(500);
			...
			DeleteAllLessonsForSelectedWeek(0);
		}
